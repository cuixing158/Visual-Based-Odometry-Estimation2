function  S = readStructBin(configFileName,binaryFileName)% 还不支持C/C++代码生成！！！， https://www.mathworks.com/matlabcentral/answers/1948868-non-constant-expression-or-empty-matrix-this-expression-must-be-constant-because-its-value-determin
...
%   cfg = "../data/preSavedData/data.cfg";
...
while ~feof(configFid)
    ...
    if flag
        ...
        % 代码生成的元胞数组限制 https://www.mathworks.com/help/fixedpoint/ug/cell-array-restrictions-for-code-generation_zh_CN.html
        % 下面代码导致end+1不可用
        % fieldLevelNames{levelID}{end+1} = iterVarName;
        % fieldLevelSizes{levelID}{end+1} = iterVarSize;
        % fieldLevelTypes{levelID}{end+1} = iterVarType;
    end
end
